...
class _RecordPageState extends State<RecordPage> {
  ...
  // paste.txt 내 fetchMonthlyActivity 함수 수정/확장
  Future<Map<int, List<Activity>>> fetchMonthlyActivity({
    ...
    if (response.statusCode == 200) {
      ...
      for (final item in json['content']) {
        final activity = Activity.fromJson(item); // 위에서 만든 팩토리 사용
        final day = DateTime.parse(item['activityDateTime']).day;
        result.putIfAbsent(day, () => []).add(activity);
      }
      return result;
    } else {
  ...
  // RecordPage.dart 내부 _initCalendar
  void _initCalendar(int year, int month, int day) async {
    setState(() => _isLoading = true);
    try {
      final data = await fetchMonthlyActivity(
        accessToken: widget.accessToken,
        year: year,
        month: month,
      ); // day 파라미터는 API에서 무시해도 OK
      setState(() {
        _currentYear = year;
        _currentMonth = month;
